...
class MolecularFF:
    ...
    def E_nonbonded(self, soft_core=False):
        ...
        if soft_core:
            ...
            r_coul = r.clamp(min=self.SOFT_CORE_COULOMB_MIN_DIST)
        else:
            r_coul = r
        ...
        # ── Coulomb ───────────────────────────────────────────────────────────
        # During soft-core (clash-removal) phase, r_coul is already clamped to
        # ≥ SOFT_CORE_COULOMB_MIN_DIST (0.3 Å by default) so that extremely
        # close atom pairs do not generate astronomical Coulomb gradients that
        # overwhelm the soft-core LJ linearisation.
